from tensorflow.keras import Model, layers, activations, regularizers

regularizer = regularizers.l2(5e-6)


def ghost_conv(x, out_channels, kernel_size, stride):
    ...
    x2 = layers.DepthwiseConv2D(kernel_size=(3, 3), strides=1, padding="same",
                                use_bias=False, kernel_regularizer=regularizer)(x2)
...
def fused_ghost_mbconv(x, in_channels, out_channels, kernel_size, activation, stride=1, expansion=6):
    shortcut = x
...
def ghost_mbconv(x, in_channels, out_channels, kernel_size, activation, stride=1, reduction_ratio=4, expansion=6):
    shortcut = x
    expanded = _depth(in_channels * expansion)
...
def repeat(x, count, in_channels, out_channels, kernel_size, activation,
    ...
    for i in range(count):
        if fused:
            x = fused_ghost_mbconv(x, in_channels, out_channels, kernel_size, activation, stride, expansion)
        else:
            x = ghost_mbconv(x, in_channels, out_channels, kernel_size, activation, stride,
...
def small(in_shape, num_classes, alpha=1.0, spatial_dropout_rate=0., dropout_rate=0.):
    ...
    x = stage(x, count=15, in_channels=_depth(160 * alpha), out_channels=_depth(272 * alpha), kernel_size=(3, 3),
              activation=activations.swish, stride=2, reduction_ratio=8, expansion=6, fused=False,
              dropout=spatial_dropout_rate)
